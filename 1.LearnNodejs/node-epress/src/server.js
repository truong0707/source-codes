...
const { Comment } = require('../models/testComments');
...
// get comment
app.get('/truong', async (req, res, next) => {
    const list = await Comment.find().exec();
    res.render('home/playmusic', {
        comments: list
    });
})
...
app.post('/truong', async (req, res) => {
    ...
    res.redirect('/truong');
    console.log("Đã thực hiện thanh công")
});
